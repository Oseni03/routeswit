...
export async function POST(
	...
	// Billing gate: ruleset count. Allow updates to an existing ruleset_id even when the org is at its limit.
	const rulesetLimitErr = await checkRulesetLimit(
		organizationId,
		ctx.tier,
		parsed.data.ruleset_id,
	);
	if (rulesetLimitErr) return rulesetLimitErr;
