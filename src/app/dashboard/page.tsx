...
export default async function DashboardPage() {
		...
		return (
			<div className="flex flex-col items-center justify-center h-[50vh]">
				...
				<p className="text-muted-foreground mt-2">
					Please select or create an organization to view your
					dashboard.
				</p>
			</div>
		);
	...
	const [
		...
		prisma.routingLog.count({
			where: { organizationId: orgId, assignedAt: { gte: monthStart } },
		}),
	...
	const leadUsagePercent =
		limits.monthly_leads === Infinity
			? 0
			: (monthlyLeads / limits.monthly_leads) * 100;
	const repUsagePercent =
		limits.reps === Infinity ? 0 : (repsCount / limits.reps) * 100;
	const rulesetUsagePercent =
		limits.rulesets === Infinity
			? 0
			: (rulesetsCount / limits.rulesets) * 100;
	...
	const stats = [
		{
			...
			value:
				limits.monthly_leads === Infinity
					? monthlyLeads.toString()
					: `${monthlyLeads} / ${limits.monthly_leads}`,
			percent: Math.min(leadUsagePercent, 100),
			icon: ContactIcon,
			color:
				leadUsagePercent >= 100 ? "text-destructive" : "text-primary",
		},
		{
			label: "Active Reps",
			value:
				limits.reps === Infinity
					? repsCount.toString()
					: `${repsCount} / ${limits.reps}`,
		...
		},
		{
			...
			value:
				limits.rulesets === Infinity
					? rulesetsCount.toString()
					: `${rulesetsCount} / ${limits.rulesets}`,
			percent: Math.min(rulesetUsagePercent, 100),
			icon: RouteIcon,
			color:
				rulesetUsagePercent >= 100
					? "text-destructive"
					: "text-primary",
		},
		{
									...
									return (
										...
										>
											...
											<div className="text-right">
												<p className="text-xs font-medium bg-green-100 text-green-700 px-2 py-1 rounded-full inline-block">
													Assigned
												</p>
												<p className="text-[10px] text-muted-foreground mt-2">
													{log.assignedAt.toLocaleString()}
												</p>
											</div>
										</div>
									);
								...
								<div className="flex gap-5 relative z-10 group/item">
									...
									<div className="space-y-0.5">
										...
										<p className="text-[11px] font-medium text-muted-foreground/70 leading-tight">
											All routing endpoints are responding
											quickly.
										</p>
								...
								{(repUsagePercent >= 90 ||
									...
									<div className="flex gap-5 relative z-10 group/item">
										...
										<div className="space-y-0.5">
											...
											<p className="text-[11px] font-medium text-muted-foreground/70 leading-tight">
												You are nearing your usage
												limits. Consider upgrading to
												Pro.
											</p>
